...
public class DowntimeWorkflowImpl implements DowntimeWorkflow {
    ...
    public DowntimeResult run(DowntimeRequest request) {
        ...
        if (request.campaignWorkflowId() != null && !request.campaignWorkflowId().isBlank()) {
            // Arrested / Show of Force may cost coin or heat — GM applies via campaign signals.
            // Stub: The Law at higher wanted often wants a heat reduction via incarceration later.
        }
    ...
    private void resolveActivity(
        ...
        switch (choice.kind()) {
            case ACQUIRE_ASSET -> {
                ...
                if (pc != null) {
                    pc.addPersonalAsset(choice.details() + " (q" + asset.quality() + ")");
                }
            }
            case LONG_TERM_PROJECT -> {
                if (pc != null) {
                    String name = choice.details() == null ? "project" : choice.details();
                    // First activity may start the clock; subsequent ticks use same name.
                    // Caller should startProject before ticking if new; here we tick 1 by default.
                    pc.applyProjectProgress(name, 1);
                }
            }
            case RECOVER -> {
                Activities.RecoveryRollResult recovery = activities.recover(Math.max(1, request.crewTier() + 1));
            ...
            case TRAIN -> {
                if (pc != null) {
                    Advancement.XpTrack track = parseTrainTrack(choice.details());
                    pc.markXp(track, 1);
                }
            }
            case INDULGE_VICE -> {
                int dice = parseViceDice(choice.details());
                DiceRoll vice = activities.rollVice(dice);
